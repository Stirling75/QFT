...
int main() {
    TFheGateBootstrappingParameterSet* params = default_new110bit_gate_bootstrapping_parameters();
    TFheGateBootstrappingSecretKeySet* key = new_random_gate_bootstrapping_secret_keyset(params);


    FILE* secret_key = fopen("secret110b_big.key", "wb");
    export_tfheGateBootstrappingSecretKeySet_toFile(secret_key, key);
    fclose(secret_key);

    FILE* cloud_key = fopen("cloud110b_big.key", "wb");
    ...
    TLweKey *TLWE_KEY = ExtracttLweKey(key, tlwe_param);
     audiofile *audio = read_audio("test_mat_4874_1024.csv");
    // audiofile *audio = read_audio("raw_random_test_mat_4874_1024.csv");
    // audiofile *audio = read_audio("test_mat_30_1024.csv");
    ...
    FILE* cloud_data = fopen("cloudMulti110b_big.data", "wb");
    for(int i=0; i<FILELENGTH; i++){
        export_tlweSample_toFile(cloud_data, &EncAudio[i], tlwe_param);
    }
    fclose(cloud_data);
    printf("\n");
